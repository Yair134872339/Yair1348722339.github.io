...
class libroControlador extends libroModelo{
    ...
    public function listar_libro_controlador($pagina, $registros, $privilegio, $id, $url, $busqueda) {
        ...
        if (isset($busqueda) && $busqueda !=""){
            $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM libro WHERE (libro_codigo LIKE '%$busqueda%' OR libro_titulo LIKE '%$busqueda%' OR libro_cliente LIKE '%$busqueda%' OR libro_editorial LIKE '%$busqueda%'OR libro_categoria LIKE '%$busqueda%') ORDER BY libro_codigo ASC LIMIT $inicio, $registros";
        } else {
            $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM libro INNER JOIN cliente ON libro.libro_cliente = cliente.cliente_id INNER JOIN editorial ON libro.libro_editorial = editorial.editorial_id INNER JOIN categoria ON libro.libro_categoria = categoria.categoria_id ORDER BY libro_codigo ASC LIMIT $inicio, $registros";
        }
    ...
    public function agregar_libro_controlador(){
        ...
        $cliente = principalModelo::limpiar_cadena($_POST['libro_cliente_guardar']);
        ...
        if ($codigo == "" || $titulo == "" || $cliente == "" || $editorial == "" || $categoria == "") {
            $alerta = [
                "Alerta" => "simple",
        ...
        if (principalModelo::verificar_datos("[0-9]{1,4}", $cliente)) {
            $alerta = [
                "Alerta" => "simple",
                "Titulo" => "¡Ocurrio un error!",
    ...
    public function actualizar_libro_controlador(){
        ...
        $cliente = principalModelo::limpiar_cadena($_POST['libro_cliente_actualizar']);
        ...
        if(principalModelo::verificar_datos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{4,30}", $cliente)){
            $alerta = [
                "Alerta"=>"simple",
                "Titulo"=>"¡Ocurrio un error!",
